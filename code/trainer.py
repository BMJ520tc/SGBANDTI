import torch
import torch.nn as nn
import copy
import os
import numpy as np
import matplotlib.pyplot as plt
from sklearn.metrics import roc_auc_score, average_precision_score, roc_curve, confusion_matrix, precision_recall_curve, precision_score, recall_score, f1_score
from torch.optim.lr_scheduler import CosineAnnealingLR
from models import binary_cross_entropy
from prettytable import PrettyTable
from tqdm import tqdm


class Trainer(object):
    def __init__(self, model, optim, device, train_dataloader, val_dataloader, test_dataloader, **config):
        self.model = model
        self.optim = optim
        self.device = device
        self.epochs = config["SOLVER"]["MAX_EPOCH"]
        self.current_epoch = 0
        self.scheduler = None
        if config["SOLVER"].get("COSINE", False):
            self.scheduler = CosineAnnealingLR(self.optim, T_max=self.epochs)
        self.train_dataloader = train_dataloader
        self.val_dataloader = val_dataloader
        self.test_dataloader = test_dataloader
        self.n_class = config["DECODER"]["BINARY"]
        self.batch_size = config["SOLVER"]["BATCH_SIZE"]
        self.nb_training = len(self.train_dataloader)
        self.step = 0

        self.best_model = None
        self.best_epoch = None
        self.best_auroc = 0
        self.val_threshold = None
        self.best_threshold = None

        self.train_loss_epoch = []
        self.val_loss_epoch, self.val_auroc_epoch = [], []
        self.test_metrics = {}
        self.config = config
        self.output_dir = config["RESULT"]["OUTPUT_DIR"]

        valid_metric_header = ["# Epoch", "AUROC", "AUPRC", "Val_loss"]
        test_metric_header = ["# Best Epoch", "AUROC", "AUPRC", "F1", "Sensitivity", "Specificity", "Accuracy",
                              "Threshold", "Test_loss"]
        train_metric_header = ["# Epoch", "Train_loss"]

        self.val_table = PrettyTable(valid_metric_header)
        self.test_table = PrettyTable(test_metric_header)
        self.train_table = PrettyTable(train_metric_header)

    def train(self):
        float2str = lambda x: '%0.4f' % x
        for i in range(self.epochs):
            self.current_epoch += 1
            train_loss = self.train_epoch()
            train_lst = ["epoch " + str(self.current_epoch)] + list(map(float2str, [train_loss]))
            self.train_table.add_row(train_lst)
            self.train_loss_epoch.append(train_loss)

            auroc, auprc, val_loss = self.test(dataloader="val")
            val_lst = ["epoch " + str(self.current_epoch)] + list(map(float2str, [auroc, auprc, val_loss]))
            self.val_table.add_row(val_lst)
            self.val_loss_epoch.append(val_loss)
            self.val_auroc_epoch.append(auroc)

            if auroc >= self.best_auroc:
                self.best_model = copy.deepcopy(self.model)
                self.best_auroc = auroc
                self.best_epoch = self.current_epoch
                self.best_threshold = self.val_threshold

            print('Validation at Epoch ' + str(self.current_epoch) + ' with validation loss ' + str(val_loss), " AUROC "
                  + str(auroc) + " AUPRC " + str(auprc))

            if self.scheduler is not None:
                self.scheduler.step()

        auroc, auprc, f1, sensitivity, specificity, accuracy, test_loss, thred_optim, precision = self.test(dataloader="test")
        test_lst = ["epoch " + str(self.best_epoch)] + list(map(float2str, [auroc, auprc, f1, sensitivity, specificity,
                                                                            accuracy, thred_optim, test_loss]))
        self.test_table.add_row(test_lst)
        print('Test at Best Model of Epoch ' + str(self.best_epoch) + ' with test loss ' + str(test_loss), " AUROC "
              + str(auroc) + " AUPRC " + str(auprc) + " Sensitivity " + str(sensitivity) + " Specificity " +
              str(specificity) + " Accuracy " + str(accuracy) + " Thred_optim " + str(thred_optim))

        self.test_metrics["auroc"] = auroc
        self.test_metrics["auprc"] = auprc
        self.test_metrics["test_loss"] = test_loss
        self.test_metrics["sensitivity"] = sensitivity
        self.test_metrics["specificity"] = specificity
        self.test_metrics["accuracy"] = accuracy
        self.test_metrics["thred_optim"] = thred_optim
        self.test_metrics["best_epoch"] = self.best_epoch
        self.test_metrics["F1"] = f1
        self.test_metrics["Precision"] = precision

        self.save_result()
        return self.test_metrics

    def save_result(self):
        if self.config["RESULT"]["SAVE_MODEL"]:
            torch.save(self.best_model.state_dict(),
                       os.path.join(self.output_dir, f"best_model_epoch_{self.best_epoch}.pth"))
            torch.save(self.model.state_dict(), os.path.join(self.output_dir, f"model_epoch_{self.current_epoch}.pth"))

        state = {
            "train_epoch_loss": self.train_loss_epoch,
            "val_epoch_loss": self.val_loss_epoch,
            "test_metrics": self.test_metrics,
            "config": self.config,
            "best_threshold": self.best_threshold
        }
        torch.save(state, os.path.join(self.output_dir, f"result_metrics.pt"))

        val_prettytable_file = os.path.join(self.output_dir, "valid_markdowntable.txt")
        test_prettytable_file = os.path.join(self.output_dir, "test_markdowntable.txt")
        train_prettytable_file = os.path.join(self.output_dir, "train_markdowntable.txt")

        with open(val_prettytable_file, 'w') as fp:
            fp.write(self.val_table.get_string())
        with open(test_prettytable_file, 'w') as fp:
            fp.write(self.test_table.get_string())
        with open(train_prettytable_file, "w") as fp:
            fp.write(self.train_table.get_string())

    def train_epoch(self):
        self.model.train()
        loss_epoch = 0
        num_batches = len(self.train_dataloader)
        for i, (v_d, v_p, labels) in enumerate(tqdm(self.train_dataloader)):
            self.step += 1
            v_d, v_p, labels = v_d.to(self.device), v_p.to(self.device), labels.float().to(self.device)
            self.optim.zero_grad()
            v_d, v_p, f, score = self.model(v_d, v_p)
            n, loss = binary_cross_entropy(score, labels)
            loss.backward()
            self.optim.step()
            loss_epoch += loss.item()
        loss_epoch = loss_epoch / num_batches
        print('Training at Epoch ' + str(self.current_epoch) + ' with training loss ' + str(loss_epoch))
        return loss_epoch

    def test(self, dataloader="test"):
        test_loss = 0
        y_label, y_pred = [], []
        if dataloader == "test":
            data_loader = self.test_dataloader
        elif dataloader == "val":
            data_loader = self.val_dataloader
        else:
            raise ValueError(f"Error key value {dataloader}")
        num_batches = len(data_loader)
        with torch.no_grad():
            self.model.eval()
            for i, (v_d, v_p, labels) in enumerate(data_loader):
                v_d, v_p, labels = v_d.to(self.device), v_p.to(self.device), labels.float().to(self.device)
                if dataloader == "val":
                    v_d, v_p, f, score = self.model(v_d, v_p)
                elif dataloader == "test":
                    v_d, v_p, f, score = self.best_model(v_d, v_p)
                n, loss = binary_cross_entropy(score, labels)
                test_loss += loss.item()
                y_label = y_label + labels.to("cpu").tolist()
                y_pred = y_pred + n.to("cpu").tolist()
        auroc = roc_auc_score(y_label, y_pred)
        auprc = average_precision_score(y_label, y_pred)
        test_loss = test_loss / num_batches

        if dataloader == "val":
            # 阈值仅在验证集上按标准 F1 选择，避免测试集泄漏
            self.val_threshold = self._select_threshold(y_label, y_pred)
            return auroc, auprc, test_loss
        else:  # dataloader == "test"
            # 使用验证集选出的阈值，测试集只评估一次
            thred_optim = self.best_threshold if self.best_threshold is not None else 0.5
            y_pred_array = np.asarray(y_pred)
            y_pred_s = (y_pred_array >= thred_optim).astype(int)
            cm1 = confusion_matrix(y_label, y_pred_s)
            accuracy = (cm1[0, 0] + cm1[1, 1]) / sum(sum(cm1))
            specificity = cm1[0, 0] / (cm1[0, 0] + cm1[0, 1])
            sensitivity = cm1[1, 1] / (cm1[1, 0] + cm1[1, 1])
            precision1 = precision_score(y_label, y_pred_s)
            f1 = f1_score(y_label, y_pred_s)

            # 保存逐样本概率/标签(与 CSV 行序一致), 供后续 paired bootstrap 与逐 seed 复现
            np.save(os.path.join(self.output_dir, "test_y_true.npy"), np.asarray(y_label))
            np.save(os.path.join(self.output_dir, "test_y_pred.npy"), np.asarray(y_pred))

            self._plot_threshold_curves(y_label, y_pred)

            return auroc, auprc, f1, sensitivity, specificity, accuracy, test_loss, thred_optim, precision1

    def _select_threshold(self, y_label, y_pred):
        """在给定集合上按标准 F1 选择最优分类阈值。"""
        precisions, recalls, thresholds = precision_recall_curve(y_label, y_pred)
        f1s = 2 * precisions * recalls / (precisions + recalls + 1e-12)
        # precision_recall_curve 末尾有一个阈值=0 的哨兵点，与 thresholds 不对齐，剔除
        best_idx = int(np.argmax(f1s[:-1]))
        return float(thresholds[best_idx])

    def _plot_threshold_curves(self, y_label, y_pred):
        """
        绘制不同阈值的AUROC和AUPRC曲线图
        """
        y_label = np.asarray(y_label)
        y_pred = np.asarray(y_pred)
        fpr, tpr, thresholds = roc_curve(y_label, y_pred)
        prec, recall, _ = precision_recall_curve(y_label, y_pred)
        plot_dir = os.path.join(self.output_dir, "plots")
        os.makedirs(plot_dir, exist_ok=True)

        metrics = []
        for threshold in thresholds:
            y_pred_s = [1 if i >= threshold else 0 for i in y_pred]
            cm = confusion_matrix(y_label, y_pred_s)
            if cm.size == 4:
                acc = (cm[0, 0] + cm[1, 1]) / sum(sum(cm)) if sum(sum(cm)) > 0 else 0
                sen = cm[1, 1] / (cm[1, 0] + cm[1, 1]) if (cm[1, 0] + cm[1, 1]) > 0 else 0
                spe = cm[0, 0] / (cm[0, 0] + cm[0, 1]) if (cm[0, 0] + cm[0, 1]) > 0 else 0
                pre = cm[1, 1] / (cm[1, 1] + cm[0, 1]) if (cm[1, 1] + cm[0, 1]) > 0 else 0
                rec = sen
                f1 = 2 * pre * rec / (pre + rec) if (pre + rec) > 0 else 0
                metrics.append([threshold, acc, sen, spe, pre, rec, f1])

        metrics = np.array(metrics)

        plt.figure(figsize=(12, 8))
        plt.plot(metrics[:, 0], metrics[:, 1], label='Accuracy')
        plt.plot(metrics[:, 0], metrics[:, 2], label='Sensitivity')
        plt.plot(metrics[:, 0], metrics[:, 3], label='Specificity')
        plt.plot(metrics[:, 0], metrics[:, 6], label='F1 Score')
        plt.xlabel('Threshold')
        plt.ylabel('Score')
        plt.title('Performance Metrics at Different Thresholds')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(plot_dir, 'threshold_metrics.png'))
        plt.close()

        plt.figure(figsize=(10, 8))
        plt.plot(fpr, tpr, label=f'ROC curve (AUC = {roc_auc_score(y_label, y_pred):.4f})')
        plt.plot([0, 1], [0, 1], 'k--')
        plt.xlabel('False Positive Rate')
        plt.ylabel('True Positive Rate')
        plt.title('Receiver Operating Characteristic (ROC) Curve')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(plot_dir, 'roc_curve.png'))
        plt.close()

        plt.figure(figsize=(10, 8))
        plt.plot(recall, prec, label=f'PR curve (AUC = {average_precision_score(y_label, y_pred):.4f})')
        plt.xlabel('Recall')
        plt.ylabel('Precision')
        plt.title('Precision-Recall (PR) Curve')
        plt.legend()
        plt.grid(True)
        plt.savefig(os.path.join(plot_dir, 'pr_curve.png'))
        plt.close()
