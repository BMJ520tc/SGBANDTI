...
class MolecularGCNStd(nn.Module):
    """
    标准 GCN 药物编码器（消融用，USE_SUBGRAPH=False 时）：
    对整分子图做多层消息传递，用真实节点加权 mean 池化，
    返回 (B, 1, hidden) 与全 1 掩码，供 BAN 以整分子向量参与交互。
    """
    def __init__(self, in_feats, dim_embedding=128, hidden=128, num_layers=3):
        super(MolecularGCNStd, self).__init__()
    ...
    def forward(self, batch_graph):
        ...
        x = torch.cat(xs, dim=1)
        batch_graph.ndata['pool_feat'] = x
        batch_graph.ndata['pool_mask'] = real_flag.unsqueeze(-1)
        # 加权 mean：只对真实节点取平均（weight 需与 feat 可广播，(N,1)）
        x_pool = dgl.mean_nodes(batch_graph, 'pool_feat', weight='pool_mask')
        x_graph = self.lin1(x_pool)
        B = x_graph.size(0)
