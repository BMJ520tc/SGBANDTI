...
def _build_dti_sample(smiles, protein_seq, label, max_drug_nodes=290, use_nested=True, h=2):
    ...
    # 只给虚拟节点补自环：smiles_to_bigraph(add_self_loop=True) 已为实节点添加自环，
    # 若再对全图 add_self_loop() 会让实节点获得重复自环，归一化与消息聚合不一致。
    if num_virtual_nodes > 0:
        virtual_ids = torch.arange(num_actual_nodes, max_drug_nodes, dtype=torch.int32)
        v_d.add_edges(virtual_ids, virtual_ids)
...
class DTIDataset(data.Dataset):
    def __init__(
        ...
        cache_root="../data/subgraph_cache",
    ):
    ...
    def _get_cache_path(self):
        mode = "nested" if self.use_nested else "flat"
    ...
    def _data_hash(self):
        """对当前使用的样本数据计算哈希，用于缓存校验（数据变化则缓存过期）。"""
        rows = self.df.iloc[self.list_IDs][["SMILES", "Protein", "Y"]]
        return hashlib.md5(rows.to_csv(index=False).encode("utf-8")).hexdigest()
    ...
    def _load_or_build_cache(self):
        ...
        if os.path.exists(cache_path) and os.path.exists(meta_path):
            ...
            print(f"数据哈希不匹配（{meta.get('data_hash','?')[:8]}≠{data_hash[:8]}），重建缓存")
