...
def ablation_tag():
    return "" if args.ablation == "full" else f"_{args.ablation}"
...
def find_best_checkpoint():
    checkpoint_dir = os.path.join(
        "./result",
        f"{args.data}_{args.split}_hop{args.hop}{ablation_tag()}",
        f"seed_{args.seed}",
    )
...
def build_test_dataloader(cfg):
    data_folder = os.path.join(f"../data/{args.data}", str(args.split))
...
def main():
    ...
    # 从训练保存的 result_metrics.pt 读取验证集阈值，复现训练时的阈值依赖指标
    metrics_path = os.path.join(checkpoint_dir, "result_metrics.pt")
    if os.path.isfile(metrics_path):
        saved = torch.load(metrics_path, map_location="cpu")
        trainer.best_threshold = saved.get("best_threshold", 0.5)
        print(f"Loaded best_threshold from {metrics_path}: {trainer.best_threshold}")
    else:
        trainer.best_threshold = 0.5
